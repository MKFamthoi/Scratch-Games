...
public class Main {
	...
	public void scaleImage() {			
		...
		for(int i = 0; i < tiles.size(); i++) {			
			Image image48 = tiles.get(i).image.getScaledInstance(
					(int)(48*gui.scale), (int)(48*gui.scale), Image.SCALE_DEFAULT);
			if(i < tiles.size()) {
	...
	public void setTitle(String s) {		
		gui.window.setTitle("Simple 2D Tile Editor version  " + version + " - " + s);
	}
	...
	public void drawOnEditor() {
		...
		if(selectedTileNum <= tiles.size()) {
			mapTileNum[worldCol][worldRow] = selectedTileNum; //TO DO			
		}
		refreshEditorImage();
